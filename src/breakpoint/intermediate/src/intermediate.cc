...
intermediate::intermediate(config * _conf, genome * g1, genome * g2)
...
int intermediate::solve()
{
	simplify_ilp();
	return 0;
}

int intermediate::simplify_ilp()
{
	//remove_tandem();
	//statistic();
...
int intermediate::statistic(string s)
{
	int f = 0;
...
int intermediate::remove_tandem()
{
	gm1->remove_tandem();
	gm2->remove_tandem();
...
int intermediate::inc_shadj(gene * x, gene * y)
{
	assert(x->x != 0 && y->x != 0);
	gene * x1 = x->a;
	gene * x2 = x->b;
...
int intermediate::print_mapping()
{
