...
trier::trier(double timelimit)
{
	...
	gm1 = new genome(conf, "G1");
	gm2 = new genome(conf, "G2");
}
...
int trier::solve()
{
	...
	printf("run intermediate ...\n");
	intermediate d(conf, gm1, gm2);
	d.solve();
...
int trier::evaluate()
{
	...
	printf("genome1: %5d out of %5d intermediates are correct. ratio = %8.3lf\n", s1, n, 1.0 * s1 / n);
	printf("genome2: %5d out of %5d intermediates are correct. ratio = %8.3lf\n", s2, n, 1.0 * s2 / n);
