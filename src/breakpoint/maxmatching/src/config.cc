...
int config::load_config(const char * conf_file)
{
	...
	while(fin.getline(buf, 1024, '\n'))
	{
		...
		else if(strcmp(key, "use_psolver")==0)
		{
			if(strcmp(value, "true") == 0) use_psolver = true;
			else use_psolver = false;
		}
		else if(strcmp(key, "remove_redundant")==0)
		{
			if(strcmp(value, "true") == 0) remove_redundant = true;
			else remove_redundant = false;
		}
	}
	return 0;
}
