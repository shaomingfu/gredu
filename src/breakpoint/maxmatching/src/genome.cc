...
genome::genome(config * _conf, const string & s)
	: conf(_conf), name(s)
{
}
...
int genome::sadist(const genome & gm, const map<gene*, gene*> & mm)
{
	...
	{
		adjacency a = v2[i];
		if(a.e1.g->x == 0) z.insert(pair<gene*, int>(a.e1.g, i));
		else if(a.e1.b == true) h.insert(pair<gene*, int>(a.e1.g, i));
		else t.insert(pair<gene*, int>(a.e1.g, i));

		if(a.e2.g->x == 0) z.insert(pair<gene*, int>(a.e2.g, i));
		else if(a.e2.b == true) h.insert(pair<gene*, int>(a.e2.g, i));
	...
	for(int i = 0; i < v1.size(); i++)
	{
		...
		if(a.e1.g->x == 0 && z.find(g1) == z.end()) continue;
		if(a.e1.g->x != 0 && a.e1.b == true && h.find(g1) == h.end()) continue;
		if(a.e1.g->x != 0 && a.e1.b == false&& t.find(g1) == t.end()) continue;

		if(a.e2.g->x == 0 && z.find(g2) == z.end()) continue;
		if(a.e2.g->x != 0 && a.e2.b == true && h.find(g2) == h.end()) continue;
		if(a.e2.g->x != 0 && a.e2.b == false&& t.find(g2) == t.end()) continue;

		int j1 = (a.e1.g->x == 0) ? z[g1] : (a.e1.b ? h[g1] : t[g1]);
		int j2 = (a.e2.g->x == 0) ? z[g2] : (a.e2.b ? h[g2] : t[g2]);

		if(j1 != j2) continue;
		assert(a.weak_compare(v2[j1]) >= 2);
...
int genome::printv(map<gene*, int> & m) const
{
	for(int i = 0; i < chrms.size(); i++)
	{
		chrm * ch = chrms.at(i);
